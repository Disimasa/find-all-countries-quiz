...
describe('ce1300 game flow', () => {

	let browser: Browser | undefined

	let baseUrl: string | null

	const answersEn = loadCe1300EnglishAnswers()
	...
	it(
		...
		'accepts a correct guess in the 1300 era',
		...
		async () => {
			...
			try {
				...
				await openPlay(page, baseUrl, 'era=ce1300&timer=0&lives=0')
				...
				const total = Number(
					...
					(await page.getByTestId('play-state').getAttribute('data-total')) ?? '0'
